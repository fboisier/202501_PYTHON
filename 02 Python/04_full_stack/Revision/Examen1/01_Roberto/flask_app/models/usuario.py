...
class Usuario:
    ...
    def get_one(cls, id):
        ...
        data = { "id": id }
        usuario = connectToMySQL('db_tutoriza').query_db(query, data)
    ...
    def get_by_email(cls, email):
        ...
        resultados = connectToMySQL('db_tutoriza').query_db(query, data)
        if len(resultados) == 0:
            return None
    ...
    def check_email(cls, email):
        ...
        results = connectToMySQL('db_tutoriza').query_db(query, data)
        quantos = len(results)
        if quantos > 0:
            email_disponivel = False
        print ()
        print ("#########################################")
        print ('Contagel email:', quantos)
        print ('email disponivel:', email_disponivel)
        print ()
